...
// Cap on how many activities get their GPS stream analyzed per sync run, to stay
// safely under Strava's API rate limits. Anything left over is picked up on the
// next "Synchroniser" click (streams_checked stays false until processed).
const MAX_STREAM_FETCHES_PER_SYNC = 60;
...
async function processBikeBestEfforts(member, accessToken) {
  ...
  for (const activity of candidates) {
    if (processed >= MAX_STREAM_FETCHES_PER_SYNC) break;
    try {

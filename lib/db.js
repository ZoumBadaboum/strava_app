...
async function init() {
  ...
  await pool.query(`ALTER TABLE activities ADD COLUMN IF NOT EXISTS streams_checked BOOLEAN NOT NULL DEFAULT false;`);

  await pool.query(`
    CREATE TABLE IF NOT EXISTS bike_best_efforts (
      id SERIAL PRIMARY KEY,
      member_id INTEGER NOT NULL REFERENCES members(id) ON DELETE CASCADE,
      ...
      UNIQUE (member_id, distance_label)
    );
  `);

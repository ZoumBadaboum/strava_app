...
async function syncMemberActivities(member) {
  ...
  const after = member.last_synced_at
    ...
    : Math.floor(Date.now() / 1000) - 60 * 60 * 24 * 730; // first sync: 2 years back
  ...
  while (true) {
    ...
    for (const a of activities) {
      await pool.query(
        `INSERT INTO activities (member_id, strava_id, name, sport_type, distance_m, elevation_m, moving_time_s, start_date)
